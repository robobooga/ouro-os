...
# Path to the capture queue - Relative to the current working directory's ouro folder
# This ensures it works when dropped into any project
QUEUE_PATH = Path.cwd() / 'ouro' / 'wiki' / 'capture-queue.md'
...
def stage(input_str):
    ...
    if not QUEUE_PATH.parent.exists():
        print(f"Error: Ouro wiki not found at {QUEUE_PATH.parent}. Run bootstrap first.")
        return
...
def crawl(directory):
    ...
    count = 0
    # Avoid crawling the wiki folder itself
    wiki_path = Path.cwd() / 'ouro' / 'wiki'
